...
class Targets:
    ...
    def load_config(self):
        ...
        if config_content is None:
            logger.warning(f'{cl.MAGENTA}Не удалось (пере)загрузить конфиг{cl.RESET}')
            return
        ...
        logger.info(f'{cl.YELLOW}Загружен конфиг {self.config} на {cl.BLUE}{len(config_targets)} целей{cl.RESET}')
